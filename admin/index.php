...
try {
    ...
    // ==========================================
    // 2. DỮ LIỆU BIỂU ĐỒ: SINH VIÊN
    // ==========================================
    $sql_sv = "SELECT 
                COALESCE(k.ten_khoa, 'Chưa phân lớp') as label_name, 
                COUNT(u.id) as so_luong
              FROM users u
              LEFT JOIN lop_hoc lh ON u.lop_hoc_id = lh.id
              LEFT JOIN khoa k ON lh.khoa_id = k.id
              WHERE u.role = 'student'
              GROUP BY k.ten_khoa
              ORDER BY so_luong DESC";
    $res_sv = $pdo->query($sql_sv)->fetchAll(PDO::FETCH_ASSOC);

    $labels_sv = [];
    $data_sv = [];
    $colors_sv = [];

    foreach ($res_sv as $row) {
        $labels_sv[] = $row['label_name'];
        $data_sv[] = $row['so_luong'];
        $colors_sv[] = ($row['label_name'] == 'Chưa phân lớp') ? 'rgba(255, 99, 132, 0.8)' : 'rgba(54, 162, 235, 0.6)';
    }

    // ==========================================
    // 3. DỮ LIỆU BIỂU ĐỒ: GIẢNG VIÊN
    // ==========================================
    $sql_gv = "SELECT 
                COALESCE(k.ten_khoa, 'Chưa phân khoa') as label_name, 
                COUNT(u.id) as so_luong
              FROM users u
              LEFT JOIN lop_hoc lh ON u.lop_hoc_id = lh.id
              LEFT JOIN khoa k ON lh.khoa_id = k.id
    ...
    foreach ($res_gv as $row) {
        ...
        $data_gv[] = $row['so_luong'];
        $colors_gv[] = ($row['label_name'] == 'Chưa phân khoa') ? 'rgba(255, 99, 132, 0.8)' : 'rgba(25, 135, 84, 0.6)';
    }
